...
6. **AI 프롬프트에 숫자 필드를 새로 넘길 때는, 이름만으로 의미가 명확하지 않다면(특히 비슷한 이름/크기의 다른 필드가 이미 존재한다면) 반드시 fieldDescriptions 패턴으로 의미를 명시한다.** 이 프로젝트에서 이미 3차례(ISA 한도 상충, generalOnly/generalForced, taxFreeLimit/납입한도) 이 유형의 버그가 발생했다. (`lib/ai/explain-simulation-result.ts`의 `TRADE_RESULT_FIELD_DESCRIPTIONS`/`buildTradeExplainPayload` 참고)
7. **금액을 원 단위가 아닌 만원/억원 등 한국어 단위로 서술해야 하는 경우, AI에게 직접 환산을 맡기지 말고 미리 포맷된 문자열을 payload에 함께 제공한다.** 원-만원 환산은 자릿수 오류(예: 100만원을 1,000만원으로 서술)가 발생하기 쉬운 지점이다(Stage 28에서 실제 발견). (`lib/ai/explain-simulation-result.ts`의 `formatManwon`/`isaTaxableExcessKrwFormatted` 참고)
8. **AI가 생성하는 텍스트에 금전적으로 정확해야 하는 숫자가 포함되어야 할 때는, AI에게 숫자를 직접 쓰게 하지 말고 `{{필드명}}` 플레이스홀더로 쓰게 한 뒤 코드에서 치환한다.** 자유 서술 중 숫자를 직접 타이핑하게 하면 payload에 정확한 값이 있어도 확률적으로 틀리게 옮겨 적을 수 있다(Stage 28의 만원 환산 오류, Stage 29의 9,900원/99,000원 오류에서 확인). (`lib/ai/explain-simulation-result.ts`의 `substitutePlaceholders`/`detectUnsubstitutedNumbers` 참고)
