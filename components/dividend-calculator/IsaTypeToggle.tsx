...
// 거치식/매매차익 도구와 동일한 결정: UI는 2단 토글(일반형/서민형)만 노출한다.
...
export function IsaTypeToggle({ isaType, onChange }: IsaTypeToggleProps) {
  ...
  const currentLimitManwon = isaType === "general" ? generalLimit : lowIncomeLimit;

  return (
    <section className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
      <div className="flex justify-between items-center mb-4">
        <span className="text-sm font-semibold text-slate-600">가입 유형 선택</span>
        <span className="text-xs font-bold text-amber-700 bg-amber-50 border border-amber-100 px-2 py-1 rounded-md">
