...
export interface TradeSimulationContext {
  ...
  response: {
    ...
    taxFreeLimitKrw: number;
    isaTaxKrw: number;
    /** Stage 27: 강제전환분(일반계좌)에 15.4%/한계세율(금융소득종합과세) 구조가 적용되면서 추가됨. */
...
// Stage 27: v4로 갱신 (매매차익 일반계좌 세율을 22% 양도소득세에서 15.4% 배당소득세+한계세율로
// 정정 — trade-calculator.ts가 다루는 대상이 국내상장 해외ETF이기 때문. v1~v3 원문은 PROMPTS.md에
// 이력으로 보존. 실사용 원문은 PROMPTS.md에도 동일하게 기록되어 있다 (지원서 제출용, 수정 시 함께
// 갱신할 것).
...
- 실제 발생 세금: ${response.totalTaxKrw.toLocaleString("ko-KR")}원 (ISA 분리과세 ${response.isaTaxKrw.toLocaleString("ko-KR")}원 + 강제전환분 배당소득세 ${response.generalForcedTaxKrw.toLocaleString("ko-KR")}원, 금융소득종합과세 대상 여부: ${response.isComprehensiveTaxationTriggered ? "예" : "아니오"}(적용 세율 ${(response.marginalTaxRateApplied * 100).toFixed(1)}%)), 전량 일반계좌였다면 ${response.generalOnlyTaxKrw.toLocaleString("ko-KR")}원, 절세액: ${response.savedAmountKrw.toLocaleString("ko-KR")}원`;
  }
