...
/**
 * Stage 8(매매차익 계산기) 전이므로 필드는 아직 정의하지 않는다.
 * 타입 구조(discriminated union의 한 축)만 미리 만들어 두고, 실제 필드와
 * buildCurrentSimulationContext의 "trade" 분기 내용은 Stage 8에서 채운다.
 */
...
export function buildCurrentSimulationContext(sim: ChatCurrentSimulation): string {
  if (sim.kind === "trade") {
    // Stage 8 전: 매매차익 계산기가 아직 없어 컨텍스트를 생성하지 않는다.
    return "";
  }
