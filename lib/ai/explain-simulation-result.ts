...
// v3 (Stage 22): generalOnlyTaxKrw(전량 일반계좌 가정 시 가상 세금)와 generalForcedTaxKrw
// (한도초과로 실제 강제전환된 세금)를 AI가 서술 중 혼동하는 문제가 Stage 21 실제 응답에서
// 발견되어(PROMPTS.md 2절 v2 응답 예시 참고), 두 필드를 명확히 구분하라는 규칙 6을 추가했다.
// v1(Stage 4)/v2(Stage 21) 원문은 PROMPTS.md에 이력으로 보존.
...
// Stage 22: trade 결과의 각 세금 필드가 정확히 무엇을 의미하는지 AI에게 명시적으로 전달한다.
// generalOnlyTaxKrw(가상 비교용)와 generalForcedTaxKrw(한도초과 강제전환분 실제 세금)를
// AI가 서술 중 혼동하는 문제가 실제로 있었기 때문에(PROMPTS.md 2절 v2 응답 예시), 이 두 필드는
// 특히 명확하게 구분해서 설명한다.
const TRADE_RESULT_FIELD_DESCRIPTIONS: Record<keyof TradeExplainInput["result"], string> = {
  ...
  isaTaxKrw:
    "ISA 편입 수량(isaQuantity)에 실제로 부과되는 세금 — 비과세 한도 초과분에 9.9% 분리과세를 적용한 값",
  generalForcedTaxKrw:
