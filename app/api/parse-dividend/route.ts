...
import { parseDividendInput } from "@/lib/ai/parse-dividend-input";
...
export async function POST(request: Request) {
  ...
  if (typeof body.text !== "string" || body.text.trim().length === 0) {
    return NextResponse.json(
      { error: "INVALID_INPUT", message: "text 필드에 자연어 배당 계획을 입력해 주세요." },
      { status: 400 }
    );
  }

  try {
    const parsed = await parseDividendInput(body.text);
    return NextResponse.json(parsed);
  } catch (error) {
    console.error("[/api/parse-dividend] AI 파싱 실패:", error);
    return NextResponse.json(
