...
## 다중 도구 구조 (Stage 7부터)
이 사이트는 여러 투자 시나리오 도구를 한 곳에서 제공한다. 상단 네비게이션으로 도구 간 이동하며, 세금 Q&A 챗봇은 세 도구가 공유한다(페이지를 옮겨도 대화 유지).
- `/` — 거치 기준 수익 시뮬레이터 (블루 테마, 위 핵심 기능 1~4)
- `/trade` — 매매차익 계산기 (그린 테마, Stage 8): 종목 매수 수량 단위로 매매차익 절세 효과 계산, 연간 납입한도 초과 시 일반계좌 강제전환 로직 포함, ISA 3년 의무유지를 가정(중도해지·금융소득종합과세는 다루지 않음)
- `/dividend` — 배당금 계산기 (앰버 테마, Stage 10): 보유 수량 기준 배당 세후 실수령액을 일반계좌 vs ISA로 비교, ISA 3년 의무유지를 가정(중도해지·ISA 납입한도 초과 로직은 다루지 않음)

자세한 도구별 가정과 제약은 [skills.md](./skills.md) 6~7절을 참고하세요.
