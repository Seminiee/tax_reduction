...
## 다중 도구 구조 (Stage 7부터, Stage 21에서 거치식/매매차익 통합)
이 사이트는 여러 투자 시나리오 도구를 한 곳에서 제공한다. 상단 네비게이션으로 도구 간 이동하며, 세금 Q&A 챗봇은 두 도구가 공유한다(페이지를 옮겨도 대화 유지).
- `/` — 절세 계좌 수익 시뮬레이터 (그린/에메랄드 테마): 종목 매수 수량 단위로 매매차익 절세 효과 계산, 연간 납입한도 초과 시 일반계좌 강제전환 로직 포함, ISA 3년 의무유지를 가정(중도해지·금융소득종합과세는 다루지 않음). AI 자연어 입력 → 결과 카드 → "AI 설명 보기" 순으로 구성(위 핵심 기능 1~4). Stage 21부터 구 거치식 시뮬레이터를 이 UI로 통합했다(거치식 계산 엔진 자체는 삭제하지 않고 보존 — skills.md 6절 참고).
- `/dividend` — 배당금 계산기 (앰버 테마, Stage 10): 보유 수량 기준 배당 세후 실수령액을 일반계좌 vs ISA로 비교, ISA 3년 의무유지를 가정(중도해지·ISA 납입한도 초과 로직은 다루지 않음)

`/trade`는 `/`로 리다이렉트된다. 자세한 도구별 가정과 제약은 [skills.md](./skills.md) 6~7절을 참고하세요.
